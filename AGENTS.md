...
4. **Execution:** Clones the repository, creates branches, and systematically attempts to patch vulnerabilities using an escalating strategy (`npm audit fix` → package overrides → manual `package.json` bumps using `uv`).
5. **Publishing:** Commits changes, pushes the fix branch, and opens a single, consolidated GitHub Pull Request.
